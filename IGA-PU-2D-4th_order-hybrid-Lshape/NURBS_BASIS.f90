...
!     print*, 'bs-here-1'
    ...
    DO J = 1, UBOUND(EXPO_ARRAY, 1)
        BY%N(BY%POLY_ORDER + J, 0) = (PAR_PT%Y*(RADI_SING(2)**(1.0D0/RLMBD)))**EXPO_ARRAY(J)
        BY%N(BY%POLY_ORDER + J, 1) = (RADI_SING(2)**(1.0D0/RLMBD))**(EXPO_ARRAY(J))*EXPO_ARRAY(J)*PAR_PT%Y**(EXPO_ARRAY(J) - 1.0D0)
        BY%N(BY%POLY_ORDER + J, 2) = (RADI_SING(2)**(1.0D0/RLMBD))**(EXPO_ARRAY(J))*EXPO_ARRAY(J)*(EXPO_ARRAY(J) - 1.0D0)*PAR_PT%Y**(EXPO_ARRAY(J) - 2.0D0)
    ENDDO
...
! ---------------------------------------------------------------------------------------------------------------------------- !
    IF (PATCH==1 .OR. (PATCH==2 .AND. PAR_PT%Y<=DELTA)) THEN
...
! 			if (patch==2 .and. indx(k)%a==2 .and. indx(k)%b==0) then 
!                 print*, PHYPU%VAL(0:2), RFG%VAL(0:2)
!             endif
		ENDDO
	ENDDO
...
TYPE(FVALUE) FUNCTION GET_BS(PAR_PT, INDX, PATCH)
    ...
    IF (PATCH==1) THEN 
        IF (INDX(2)>BASIS_KVEC(PATCH, 2)%LENGTH - BS_ORDER(PATCH, 2)-1) THEN 
            NY_INDX = INDX(2) - (BASIS_KVEC(PATCH, 2)%LENGTH - BS_ORDER(PATCH, 2)-1)
            NY%VAL(0) = (PAR_PT%Y*(RADI_SING(2)**(1.0D0/RLMBD)))**EXPO_ARRAY(NY_INDX)
            NY%VAL(1) = (RADI_SING(2)**(1.0D0/RLMBD))**(EXPO_ARRAY(NY_INDX))*EXPO_ARRAY(NY_INDX)*PAR_PT%Y**(EXPO_ARRAY(NY_INDX) - 1.0D0)
            NY%VAL(2) = (RADI_SING(2)**(1.0D0/RLMBD))**(EXPO_ARRAY(NY_INDX))*EXPO_ARRAY(NY_INDX)*(EXPO_ARRAY(NY_INDX) - 1.0D0)*PAR_PT%Y**(EXPO_ARRAY(NY_INDX) - 2.0D0)
        ELSE
            NY = GET_DIFF_BSPLINE(PAR_PT%Y, BASIS_KVEC(PATCH, 2), INDX(2), 2)
        ENDIF
	ELSE 
