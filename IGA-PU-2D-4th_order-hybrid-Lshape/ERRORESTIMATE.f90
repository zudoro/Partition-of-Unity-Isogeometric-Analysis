	...
	DO PATCH = 1, NUMPATCH
        DO I = 0, 100
            DO J = 0, 100
                ...
                IF (PATCH==1 .AND. R>=RADI_SING(1) .AND. R<=RADI_SING(2)) THEN 
                ELSE 
...
SUBROUTINE ENRGY(ENRG, DMY_K, SOL)
    ...
    IF (PROBLEM==1) THEN
        TR = 6.27209727767352198670d0
    elseif (PROBLEM==3) then 
        tr = 6.49220044155994457920d0
    elseif (problem==4) then 
        tr = 0.1828571428571427460d0
    ENDIF
