...
! C
        ...
        REAL*8, INTENT(OUT) :: BI(0:N),DI(0:N),BK(0:N),DK(0:N)
...
10            DK(K)=-1.0D+300
        ...
        DK(1)=DK1
        IF (N.LE.1) RETURN
        IF ((X-40.D0).GE.1.5D-15 .AND. N.LT.INT(0.25*X)) THEN
           H0=BI0
           H1=BI1
           DO 15 K=2,N
...
30         BI1=0.5D0*X*BI1
        ELSE
           DATA A/0.125D0,7.03125D-2,7.32421875D-2,1.1215209960938D-1,2.2710800170898D-1,5.7250142097473D-1,1.7277275025845D0,6.0740420012735D0,2.4380529699556D01,1.1001714026925D02,5.5133589612202D02,3.0380905109224D03/
           DATA B/-0.375D0,-1.171875D-1,-1.025390625D-1,-1.4419555664063D-1,-2.7757644653320D-1,-6.7659258842468D-1,-1.9935317337513D0,-6.8839142681099D0,-2.7248827311269D01,-1.2159789187654D02,-6.0384407670507D02,-3.3022722944809D03/
           K0=12
           IF (X.GE.35.0) K0=9
           IF (X.GE.50.0) K0=7
...
70         BK0=BK0+CT
        ELSE
           DATA A1/0.125D0,0.2109375D0,1.0986328125D0,1.1775970458984D01,2.1461706161499D02,5.9511522710323D03,2.3347645606175D05,1.2312234987631D07/
           CB=0.5D0/X
           XR2=1.0D0/X2
           BK0=1.0D0
           DO 75 K=1,8
...
! C
        ...
        DO 10 IT=1,20
           NN=N1-(N1-N0)/(1.0D0-F0/F1)
					 !PRINT*,IT, NN, N1, F0, F1, (N1-N0)/(1.0D0-F0/F1)
           F=ENVJ(NN,A0)-OBJ
           IF (ABS(NN-N1).LT.1) GO TO 20
           N0=N1
           F0=F1
