...
CONTAINS
	...
	TYPE(POINT2D) FUNCTION GET_REF_PT(PHY_PT)
		...
		ERROR = 1.0D0
		OLD_PT = POINT2D(0.5D0 + DSQRT(2.0D0)*EPS, 0.5D0 - DSQRT(2.0D0)*EPS)
		ITER = 0
...
! 			PRINT*, 'OLD_PT : ', OLD_PT
			...
			ELSE
				...
				OLD_PT = NEW_PT
				OLD_PT%X = MIN(1.0D0 - DSQRT(2.0D0)*EPS, MAX(EPS,OLD_PT%X))
				OLD_PT%Y = MIN(1.0D0 - EPS, MAX(DSQRT(2.0D0)*EPS,OLD_PT%Y))
			ENDIF
		ENDDO
