...
! 	ENDIF
		...
		DO J1 = 0, BASIS_KVEC(1)%POLY_ORDER
			...
			IF (INDX_X>=0) THEN
				DO J2 = 0, BASIS_KVEC(2)%POLY_ORDER
					...
					IF (INDX_Y>=0) THEN
						DWSUM(K,L) = DWSUM(K,L) + DBSFUN(1)%N(J1,K)*DBSFUN(2)%N(J2,L)*GEO_CTL%WGTS(INDX_X,INDX_Y,0)
					ENDIF
				ENDDO
			ENDIF
		ENDDO
	...
	DO I2 = 0, BASIS_KVEC(2)%POLY_ORDER
		...
		DO L = 0, DIFF_ORDER-K
			...
			DIFF_NURBS%VAL(K,L) = DBSFUN(1)%N(I1,K)*DBSFUN(2)%N(I2,L)*GEO_CTL%WGTS(INDX_X,INDX_Y,0)
...
TYPE(FVALUE) FUNCTION GET_PHY_NURBS_SURFACE_2D(REF_PT, IX, IY, JACOB)
	...
	DO L = 0, DIFF_ORDER-K
		...
		DIFF_NURBS%VAL(K,L) = REF_SF%VAL(K,L)*GEO_CTL%WGTS(IX,IY,0)
