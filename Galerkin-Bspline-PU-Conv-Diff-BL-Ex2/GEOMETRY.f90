...
!! BASIS FUNCTION ON THE OMEGA_HAT1
	...
	BASIS_KVEC(1,1,1)%LENGTH = 5
	BASIS_KVEC(1,1,2)%LENGTH = 5
	
	BASIS_KVEC(1,1,1)%KNOTS(0:BASIS_KVEC(1,1,1)%LENGTH) = (/0.D0,0.D0,0.D0,1.D0,1.D0,1.D0/)
	...
	BASIS_CTL(1,1)%PTS(1,0,0) = POINT2D(0.50D0, PATCHBDY(1,1)-DELTA)
	BASIS_CTL(1,1)%PTS(2,0,0) = POINT2D(1.0D0, PATCHBDY(1,1)-DELTA)
	BASIS_CTL(1,1)%PTS(0,1,0) = POINT2D(0.0D0, 0.50D0*SUM(PATCHBDY(1,:)))
	BASIS_CTL(1,1)%PTS(1,1,0) = POINT2D(0.50D0, 0.50D0*SUM(PATCHBDY(1,:)))
	BASIS_CTL(1,1)%PTS(2,1,0) = POINT2D(1.0D0, 0.50D0*SUM(PATCHBDY(1,:)))
	BASIS_CTL(1,1)%PTS(0,2,0) = POINT2D(0.0D0, PATCHBDY(1,2))
	BASIS_CTL(1,1)%PTS(1,2,0) = POINT2D(0.50D0, PATCHBDY(1,2))
	BASIS_CTL(1,1)%PTS(2,2,0) = POINT2D(1.0D0, PATCHBDY(1,2))
...
! 	NUMBS(:) = (/BASIS_KVEC(1)%LENGTH - BS_ORDER(1)-1, BASIS_KVEC(2)%LENGTH - BS_ORDER(2)/) ! IMPOSED PERIODIC CONDTION
	...
	DO PATCH_ROW = 1, NUMPATCH(1)
		DO PATCH_COLUMN = 1, NUMPATCH(2)
! 			IF (PATCH_ROW==1 .AND. (PATCH_COLUMN==2 .OR. PATCH_COLUMN==3)) THEN ! IMPOSED PERIODIC CONTION ON OMEGA_HAT2 AND OMEGA_HAT3 IN XI-DIRECTION
				NUMBS(1) = NUMBS(1) + BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 1)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN,1) - 1
				LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 1) = BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 1)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN, 1) - 1
				NUMBS(2) = NUMBS(2) + BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 2)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN,2)
				LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 2) = BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 2)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN, 2)
! 			ELSE 
! 				DO K = 1, 2
! 					NUMBS(K) = NUMBS(K) + BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, K)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN,K)
! 					LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, K) = BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, K)%LENGTH - BS_ORDER(PATCH_ROW, PATCH_COLUMN,K)
! 				ENDDO
! 			ENDIF
		ENDDO
	ENDDO
	...
	DO PATCH_ROW = 1, NUMPATCH(1)
		DO PATCH_COLUMN = 1, NUMPATCH(2)
			DOF = DOF + PRODUCT(LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, :))
		ENDDO
	ENDDO
...
!-----------------------------------------------------------------------------------------------------------
	...
	DO PATCH_ROW = 1, NUMPATCH(1)
		DO PATCH_COLUMN = 1, NUMPATCH(2)
			DO J = 1, PRODUCT(LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, :))
! 				NDX(1,I) = 1 + INT((I-1)/(LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 1)*LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 2)))
				NDX(1,I) = PATCH_ROW
				NDX(2,I) = PATCH_COLUMN
				NDX(3,I) = MOD(J + (LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 1)-1), LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 1))
				NDX(4,I) = MOD(INT((J-1)/LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 1)) + LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 2), LOC_NUMBS(PATCH_ROW, PATCH_COLUMN, 2))
				I = I + 1
			ENDDO
		ENDDO
	ENDDO
...
! 	DO PATCH_ROW = 1, NUMPATCH(1)
		DO PATCH_COLUMN = 1, NUMPATCH(2)
			DO J = 1, 2
				K = 1
				DO I = 1, BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, J)%LENGTH
					K = K + 1
					IF (DABS(BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, J)%KNOTS(I-1) - BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, J)%KNOTS(I)).LE.EPS) THEN
						K = K - 1
					ELSE
						IF (J==1) THEN
							PARPT = UNITSQ_TO_PARSP(BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, J)%KNOTS(I), (/PATCH_ROW, PATCH_COLUMN/), 'X')
						ELSEIF (J==2) THEN
							PARPT = UNITSQ_TO_PARSP(BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, J)%KNOTS(I), (/PATCH_COLUMN, PATCH_COLUMN/), 'Y')
						ENDIF
						IR_GRID(PATCH_ROW, PATCH_COLUMN, J, K) = PARPT%VAL(0)
					ENDIF
				ENDDO
				NUMIR(PATCH_ROW, PATCH_COLUMN, J) = K
			ENDDO
			PARPT = UNITSQ_TO_PARSP(BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 1)%KNOTS(0), (/PATCH_ROW, PATCH_COLUMN/), 'X')
			IR_GRID(PATCH_ROW, PATCH_COLUMN, 1, 1) = PARPT%VAL(0)
			PARPT = UNITSQ_TO_PARSP(BASIS_KVEC(PATCH_ROW, PATCH_COLUMN, 2)%KNOTS(0), (/PATCH_ROW, PATCH_COLUMN/), 'Y')
			IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, 1) = PARPT%VAL(0)
		ENDDO
! 	ENDDO

!! INSERT POINTS CORRESPONDING TO END POINTS OF THE INTERVAL OF OVERLAPPING PART OF PU FUNCTIONS
! ETA-DIRECTION
	DO PATCH_ROW = 1, NUMPATCH(1)
		DO PATCH_COLUMN = 1, NUMPATCH(2)
			K = 1
			TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, 1) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, 1)
			IF (PATCH_ROW==1.AND.PATCH_COLUMN==1) THEN
				DO I = 2, NUMIR(PATCH_ROW, PATCH_COLUMN, 2)
					K = K + 1
					IF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1) .AND. &
							IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,1) .AND. &
							IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)<PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
							
							dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-PATCHBDY(PATCH_COLUMN,1))>eps .AND. &
							dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-PATCHBDY(PATCH_COLUMN,1))>eps .AND. &
							dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps) THEN
							
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN, 1)
						K = K + 1
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
						
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1) .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,1)+DELTA .and. &
									
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-PATCHBDY(PATCH_COLUMN,1))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps) THEN
							
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN, 1) + DELTA
						K = K + 1
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1) .AND. &
						...
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN, 1)
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+1) = PATCHBDY(PATCH_COLUMN, 1) + DELTA
						K = K + 2
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ENDIF
				ENDDO
				NUMIR(PATCH_ROW, PATCH_COLUMN, 2) = K
			ELSEIF (PATCH_ROW==1.AND.PATCH_COLUMN==2) THEN
				DO I = 2, NUMIR(PATCH_ROW, PATCH_COLUMN, 2)
					K = K + 1
					IF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1) .AND. &
							IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,1) .AND. &
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1) .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)<PATCHBDY(PATCH_COLUMN,2)-DELTA .and. &
									
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-PATCHBDY(PATCH_COLUMN,1))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-(PATCHBDY(PATCH_COLUMN,2)-DELTA))>eps) THEN
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,2)-DELTA .AND.&
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,2)-DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)<PATCHBDY(PATCH_COLUMN,2) .and. &
									
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-(PATCHBDY(PATCH_COLUMN,1)+DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-(PATCHBDY(PATCH_COLUMN,2)-DELTA))>eps .AND.&
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-(PATCHBDY(PATCH_COLUMN,2)-DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-PATCHBDY(PATCH_COLUMN,2))>eps) THEN
					
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,2) - DELTA
						K = K + 1
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1)+DELTA .AND. &
						...
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,2) - DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+1) = PATCHBDY(PATCH_COLUMN,2)
						K = K + 2
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
						
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,2)-DELTA .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,2) .AND. &
									IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,2) .and. &
									
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-(PATCHBDY(PATCH_COLUMN,2)-DELTA))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)-PATCHBDY(PATCH_COLUMN,2))>eps .AND. &
									dabs(IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)-PATCHBDY(PATCH_COLUMN,2))>eps) THEN
					
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,2)
						K = K + 1
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,1) .AND. &
						...
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,1)
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+1) = PATCHBDY(PATCH_COLUMN,1) + DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+2) = PATCHBDY(PATCH_COLUMN,2) - DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+3) = PATCHBDY(PATCH_COLUMN,2)
						K = K + 4
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1) .AND. &
						...
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,1) + DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+1) = PATCHBDY(PATCH_COLUMN,2) - DELTA
						K = K + 2
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
					...
					ELSEIF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)>PATCHBDY(PATCH_COLUMN,1) .AND. &
						...
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K) = PATCHBDY(PATCH_COLUMN,1) + DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+1) = PATCHBDY(PATCH_COLUMN,2) - DELTA
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2, K+2) = PATCHBDY(PATCH_COLUMN,2)
						K = K + 3
						TMPIR_GRID(PATCH_ROW, PATCH_COLUMN, 2,K) = IR_GRID(PATCH_ROW, PATCH_COLUMN, 2,I)
				...
				ENDDO
				NUMIR(PATCH_ROW, PATCH_COLUMN, 2) = K
			ELSEIF (PATCH_ROW==1.AND.PATCH_COLUMN==3) THEN
				DO I = 2, NUMIR(PATCH_ROW, PATCH_COLUMN, 2)
					K = K + 1
					IF (IR_GRID(PATCH_ROW, PATCH_COLUMN, 2, I-1)<PATCHBDY(PATCH_COLUMN,2)-DELTA .AND. &
							IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)>PATCHBDY(PATCH_COLUMN,2)-DELTA .AND. &
							IR_GRID(PATCH_ROW, PATCH_COLUMN,2,I)<PATCHBDY(PATCH_COLUMN,2) .and. &
		...
		ENDDO
	ENDDO
	IR_GRID(:,:,2,:) = TMPIR_GRID(:,:,2,:)
	
	TMPIR_GRID(:,:,1,:) = IR_GRID(:,:,1,:)
	
	do j = 1, numpatch(2)
		patches(1) = j
		do jj = 1, numpatch(2)
			patches(2) = jj
			if (patches(1)==patches(2)) then
				goto 156
			endif
			DO K = 1, 2
				kk = 1
				TMPIR_GRID(1, PATCHES(1), K, 1) = IR_GRID(1, PATCHES(1), K, 1)
				DO II = 2, NUMIR(1, PATCHES(1), K)
					DO I = 1, NUMIR(1, PATCHES(2), K)
						IF (IR_GRID(1,PATCHES(1),K,1)<IR_GRID(1, PATCHES(2), K, I) .AND. IR_GRID(1, PATCHES(2), K, I)<IR_GRID(1,PATCHES(1),K,NUMIR(1,PATCHES(1),K)) .AND. &
								IR_GRID(1,PATCHES(1),K,II-1)<IR_GRID(1, PATCHES(2), K, I) .AND. IR_GRID(1, PATCHES(2), K, I)<IR_GRID(1,PATCHES(1), K, II) .AND. &
								DABS(IR_GRID(1,PATCHES(1),K,1) - IR_GRID(1, PATCHES(2), K, I))>EPS .AND. DABS(IR_GRID(1, PATCHES(2), K, I) - IR_GRID(1,PATCHES(1),K,NUMIR(1,PATCHES(1),K)))>EPS .AND. &
								DABS(IR_GRID(1,PATCHES(1),K,II-1) - IR_GRID(1, PATCHES(2), K, I))>EPS .AND. DABS(IR_GRID(1, PATCHES(2), K, I) - IR_GRID(1,PATCHES(1), K, II))>EPS) THEN
							KK = KK + 1
							TMPIR_GRID(1, PATCHES(1), K, KK) = IR_GRID(1, PATCHES(2), K, I)
		! 							PRINT*, 'J',J,'JJ',J,'K',K,'KK',KK,TMPIR_GRID(1, PATCHES(1), K, KK)
						ENDIF
					ENDDO
					KK = KK + 1
					TMPIR_GRID(1, PATCHES(1), K, KK) = IR_GRID(1, PATCHES(1), K, II)
		! 					print*, KK, TMPIR_GRID(1, PATCHES(1), K, KK), II, IR_GRID(1, PATCHES(1), K, II)
				ENDDO
				NUMIR(1, PATCHES(1), K) = kk
			ENDDO
			IR_GRID(1, patches(1), :, :) = TMPIR_GRID(1, patches(1), :, :)
			156 continue
		enddo
	enddo
...
! 	IF (ZEROBD=='Y') THEN
		IF (PDE=='CNVD' .AND. PROBLEM==4) THEN
			DO I=1, DOF
!					IF (NDX(2,I)==3 .AND. (NDX(3,I)<=INT(0.5*(LOC_NUMBS(1, 3, 1)-1)) .OR. NDX(3,I)==LOC_NUMBS(1,3,1)-1) .AND. NDX(4,I)==0) THEN
! 				IF (NDX(2,I)==3 .AND. NDX(3,I)>0 .AND. NDX(3,I)<INT(0.5*(LOC_NUMBS(1, 3, 1)-1)) .AND. NDX(4,I)==0) THEN
				IF (NDX(2,I)==3 .AND. (NDX(3,I)==0 .OR. NDX(3,I)>=INT(0.5*(LOC_NUMBS(1, 3, 1)-1))) .AND. NDX(4,I)==0) THEN
					K = K + 1
