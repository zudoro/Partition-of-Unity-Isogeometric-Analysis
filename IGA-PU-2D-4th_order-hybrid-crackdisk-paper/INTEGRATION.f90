			...
			DO II=1, NUMIR(PATCH_ROW, PATCH_COLUMN, 1)-1
				...
				DO I = 1, NUMGSPT
					DO J = 1, NUMGSPT
						...
						GSPT = POINT2D(GSX(I), GSY(J))
                        !---------------------------------------------!
                        PHYPT = GET_PHY_PT(GSPT, 1)
                        PARPT = GET_PAR_PT(PHYPT, 2)
                        !---------------------------------------------!
						JACOB(1) = GET_JACOBIAN_MATRIX(GSPT, 1)
						JACOB(2) = GET_JACOBIAN_MATRIX(PARPT, 2)
						DET_M = .DETERMINANT.JACOB(1)
						WEIGHT(KK) = DET_M*GSXW(I)*GSYW(J)
                        IF (PATCH==1) THEN
                            CALL GET_PATCHWISE_BS_HYBRID(SF_ROW(KK, :), INDX_ROW(:), GSPT, PATCH_ROW, JACOB(1))     ! GET_PATCHWISE_BS(SF, INDX, PAR_PT, PATCH, JACOB)
                            CALL GET_PATCHWISE_BS(SF_COLUMN(KK, :), INDX_COLUMN(:), PARPT, PATCH_COLUMN, JACOB(2))     ! GET_PATCHWISE_BS(SF, INDX, PAR_PT, PATCH, JACOB)
                        ELSEIF (PATCH==2) THEN
                            CALL GET_PATCHWISE_BS(SF_ROW(KK, :), INDX_ROW(:), PARPT, PATCH_ROW, JACOB(2))     ! GET_PATCHWISE_BS(SF, INDX, PAR_PT, PATCH, JACOB)
                            CALL GET_PATCHWISE_BS_HYBRID(SF_COLUMN(KK, :), INDX_COLUMN(:), GSPT, PATCH_COLUMN, JACOB(1))     ! GET_PATCHWISE_BS(SF, INDX, PAR_PT, PATCH, JACOB)
                        ENDIF
					ENDDO
				ENDDO
				!--------------------EVALUATE INTEGRAND AT EACH GAUSS POINT IN SUBMATRIX-----------
