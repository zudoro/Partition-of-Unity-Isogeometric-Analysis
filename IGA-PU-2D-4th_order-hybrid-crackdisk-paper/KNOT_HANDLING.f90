...
! 			PRINT*, 'MH',MH,'PH',PH
	...
	SUBROUTINE REMOVE_KNOT(OLD_KVEC, OLD_CTL, KICK_NDX, KICK_MULTIPLICITY, NUM, AXIS)
		
		TYPE(KNOT_VECTOR), INTENT(INOUT) :: OLD_KVEC(2)
		TYPE(CONTROL_POINTS_2D), INTENT(INOUT) :: OLD_CTL
		INTEGER, INTENT(IN) :: KICK_NDX, KICK_MULTIPLICITY, NUM
		CHARACTER(LEN=1), INTENT(IN) :: AXIS
		
		REAL*8 :: U, ALFI, ALFJ
		INTEGER :: M(2), N(2), ORD(2), R, S, P, FOUT, LAST, FIRST, T, OFF
		INTEGER :: I, J, K, II, JJ, KK, REMFLAG
		TYPE(CONTROL_POINTS_2D) :: TEMP
		
! 		DO I = 0, MAX_ORDER
! 			DO J = 0, MAX_ORDER
! 				OLD_CTL%PTS(I,J,0)%X = OLD_CTL%PTS(I,J,0)%X*OLD_CTL%WGTS(I,J,0)
! 				OLD_CTL%PTS(I,J,0)%Y = OLD_CTL%PTS(I,J,0)%Y*OLD_CTL%WGTS(I,J,0)
! 			ENDDO
! 		ENDDO
...
! 		PRINT*, 'N : ', N
		...
		IF (AXIS=='X') THEN
			...
			U = OLD_KVEC(1)%KNOTS(R)
			P = OLD_KVEC(1)%POLY_ORDER
			
			FOUT = (2*R - S - P)/2
			LAST = R - S
			FIRST = R - P
			
			DO T = 0, NUM-1 ! This loop is Eq. (5.28)
				OFF = FIRST - 1
				
				TEMP%PTS(0,:,0)%X = OLD_CTL%PTS(OFF,:,0)%X
				TEMP%PTS(0,:,0)%Y = OLD_CTL%PTS(OFF,:,0)%Y
				TEMP%WGTS(0,:,0) = OLD_CTL%WGTS(OFF,:,0)
				
				TEMP%PTS(LAST + 1 - OFF,:,0)%X = OLD_CTL%PTS(LAST + 1,:,0)%X
				TEMP%PTS(LAST + 1 - OFF,:,0)%Y = OLD_CTL%PTS(LAST + 1,:,0)%Y
				TEMP%WGTS(LAST + 1 - OFF,:,0) = OLD_CTL%WGTS(LAST + 1,:,0)
				
				I = FIRST; J = LAST
				II = 1; JJ = LAST - OFF
				REMFLAG = 0
				
				DO WHILE ((J - I)>T) ! Compute new control points for one removal step
					
					ALFI = (U - OLD_KVEC(1)%KNOTS(I))/(OLD_KVEC(1)%KNOTS(I+ORD(1)+T) - OLD_KVEC(1)%KNOTS(I))
					ALFJ = (U - OLD_KVEC(1)%KNOTS(J-T))/(OLD_KVEC(1)%KNOTS(J+ORD(1)) - OLD_KVEC(1)%KNOTS(J-T))
					
					DO K = 0, N(2)
						TEMP%PTS(II,K,0)%X = (OLD_CTL%PTS(I,K,0)%X - (1.0D0 - ALFI)*TEMP%PTS(II-1,K,0)%X)/ALFI
						TEMP%PTS(II,K,0)%Y = (OLD_CTL%PTS(I,K,0)%Y - (1.0D0 - ALFI)*TEMP%PTS(II-1,K,0)%Y)/ALFI
						TEMP%WGTS(II,K,0) = (OLD_CTL%WGTS(I,K,0) - (1.0D0 - ALFI)*TEMP%WGTS(II-1,K,0))/ALFI
						
						TEMP%PTS(JJ,K,0)%X = (OLD_CTL%PTS(J,K,0)%X - ALFJ*TEMP%PTS(JJ+1,K,0)%X)/(1.0D0 - ALFJ)
						TEMP%PTS(JJ,K,0)%Y = (OLD_CTL%PTS(J,K,0)%Y - ALFJ*TEMP%PTS(JJ+1,K,0)%Y)/(1.0D0 - ALFJ)
						TEMP%WGTS(JJ,K,0) = (OLD_CTL%WGTS(J,K,0) - ALFJ*TEMP%WGTS(JJ+1,K,0))/(1.0D0 - ALFJ)
					ENDDO
				...
				IF ((J-I)<T) THEN ! Check if knot removable
					IF (DSQRT((TEMP%PTS(II-1,N(2),0)%X - TEMP%PTS(JJ+1,N(2),0)%X)**2 + &
					          (TEMP%PTS(II-1,N(2),0)%Y - TEMP%PTS(JJ+1,N(2),0)%Y)**2 + &
					          (TEMP%WGTS(II-1,N(2),0) - TEMP%WGTS(JJ+1,N(2),0))**2)<=EPS) THEN
						REMFLAG = 1
					ENDIF
				ELSE
					ALFI = (U-OLD_KVEC(1)%KNOTS(I))/(OLD_KVEC(1)%KNOTS(I+ORD(1)+T)-OLD_KVEC(1)%KNOTS(I))
					IF (DSQRT((OLD_CTL%PTS(I,N(2),0)%X - (ALFI*TEMP%PTS(II+T+1,N(2),0)%X + (1.0D0-ALFI)*TEMP%PTS(II-1,N(2),0)%X))**2 &
									+ (OLD_CTL%PTS(I,N(2),0)%Y - (ALFI*TEMP%PTS(II+T+1,N(2),0)%Y + (1.0D0-ALFI)*TEMP%PTS(II-1,N(2),0)%Y))**2 &
									+ (OLD_CTL%WGTS(I,N(2),0) - (ALFI*TEMP%WGTS(II+T+1,N(2),0) + (1.0D0-ALFI)*TEMP%WGTS(II-1,N(2),0)))**2)<=EPS) THEN
						REMFLAG = 1
					ENDIF
				ENDIF
				...
				ELSE
					...
					DO WHILE ((J-I)>T)
						print*, 'here-1'
						DO K = 0, N(2)
							OLD_CTL%PTS(I,K,0)%X = TEMP%PTS(I-OFF,K,0)%X
							OLD_CTL%PTS(I,K,0)%Y = TEMP%PTS(I-OFF,K,0)%Y
							OLD_CTL%WGTS(I,K,0) = TEMP%WGTS(I-OFF,K,0)
							
							OLD_CTL%PTS(J,K,0)%X = TEMP%PTS(J-OFF,K,0)%X
							OLD_CTL%PTS(J,K,0)%Y = TEMP%PTS(J-OFF,K,0)%Y
							OLD_CTL%WGTS(J,K,0) = TEMP%WGTS(J-OFF,K,0)
						ENDDO
			...
			DO K = R+1, M(1)
				OLD_KVEC(1)%KNOTS(K-T) = OLD_KVEC(1)%KNOTS(K)
			ENDDO
			
			OLD_KVEC(1)%LENGTH = OLD_KVEC(1)%LENGTH - T
			J = FOUT; I = J
			print*, 'fout :', fout, 'i :', i
			DO K = 1, T-1
				IF (MOD(K,2)==1) THEN
			...
			DO K = I + 1, N(1)
				DO KK = 0, N(2)
					OLD_CTL%PTS(J,KK,0)%X = OLD_CTL%PTS(K,KK,0)%X
					OLD_CTL%PTS(J,KK,0)%Y = OLD_CTL%PTS(K,KK,0)%Y
					OLD_CTL%WGTS(J,KK,0) = OLD_CTL%WGTS(K,KK,0)
				ENDDO
				J = J + 1
			ENDDO
...
! 			ENDDO
			...
			PRINT*, 'Remove knot(s) along xi - direction'
